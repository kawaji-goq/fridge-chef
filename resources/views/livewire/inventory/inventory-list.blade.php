<div class="mx-auto max-w-md p-4 space-y-6">
    ...
    <section class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 p-4 space-y-4">
        ...
        <div class="relative">
            ...
            @if($selectedIngredientId)
                <p class="mt-1 text-xs text-rose-700">
                    選択中：<strong>{{ $selectedIngredientName }}</strong>
                    <button type="button" wire:click="clearIngredient" class="underline ml-2">変更</button>
                </p>
            ...
            <div class="grid grid-cols-3 gap-2">
                @foreach(['fridge' => '冷蔵', 'freezer' => '冷凍', 'pantry' => '常温'] as $val => $label)
                    <label class="flex items-center justify-center rounded-lg border px-3 py-2 cursor-pointer text-sm font-medium
                        {{ $storageLocation === $val ? 'bg-rose-200 text-rose-900 border-rose-200' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
        ...
        <button
            ...
            class="block w-full rounded-lg bg-rose-200 text-rose-900 font-semibold py-3 hover:bg-rose-300 disabled:opacity-50"
        >
    ...
    <section class="space-y-3">
        ...
        @forelse($this->items as $item)
            @if($editingItemId === $item->id)
                {{-- 編集モード --}}
                <article class="rounded-xl bg-white shadow-sm ring-1 ring-rose-200 p-4 space-y-3">
                    <div class="flex items-center gap-2">
                        <span class="inline-block rounded-full px-2 py-0.5 text-xs font-medium bg-rose-100 text-rose-800">編集中</span>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">数量</label>
                            <input type="number" step="0.01" min="0" wire:model="editQuantity"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-400 focus:ring-rose-300 text-base">
                            @error('editQuantity') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">単位</label>
                            <select wire:model="editUnitId"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-400 focus:ring-rose-300 text-base">
                                @foreach($this->units as $u)
                                    <option value="{{ $u->id }}">{{ $u->label_ja }}</option>
                                @endforeach
                            </select>
                            @error('editUnitId') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            @foreach(['fridge' => '冷蔵', 'freezer' => '冷凍', 'pantry' => '常温'] as $val => $label)
                                <label class="flex items-center justify-center rounded-lg border px-3 py-2 cursor-pointer text-sm font-medium
                                    {{ $editStorageLocation === $val ? 'bg-rose-200 text-rose-900 border-rose-200' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                                    <input type="radio" wire:model.live="editStorageLocation" value="{{ $val }}" class="sr-only" />
                                    {{ $label }}
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">期限（任意）</label>
                        <div class="grid grid-cols-2 gap-3">
                            <input type="date" wire:model="editExpiresAt"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-400 focus:ring-rose-300 text-base">
                            <select wire:model="editExpiresType"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-rose-400 focus:ring-rose-300 text-base">
                                <option value="best_before">賞味期限</option>
                                <option value="use_by">消費期限</option>
                            </select>
            ...
            @else
                ...
                <article class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 p-3 flex items-center justify-between gap-3">
                    <button
                        ...
                        class="min-w-0 flex-1 text-left -m-3 p-3 rounded-xl hover:bg-rose-50 transition"
                    >
                        <div class="flex items-center gap-2">
                    ...
                    <button
                        ...
                        wire:confirm="削除しますか？"
                        class="text-red-600 hover:text-red-800 p-2 shrink-0"
                        aria-label="削除"
                    >
